...
class Case(Base):
    ...
    created_by_fund_id = Column(UUID(as_uuid=True), ForeignKey("fund.id"), nullable=False)
    created_by_fund = relationship("Fund")
    
    closed_at = Column(DateTime(timezone=True), nullable=True)
    closed_by_user_id = Column(UUID(as_uuid=True), ForeignKey("user.id"), nullable=True)
